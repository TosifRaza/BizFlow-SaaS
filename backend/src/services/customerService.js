...
const getAll = async (businessId, query) => {
  ...
  // Compute totalPurchases from Sales for these customers
  const customerIds = customers.map(c => c._id);
  let purchaseMap = {};
  if (customerIds.length > 0) {
    try {
      const purchaseData = await Sale.aggregate([
        { $match: { customerId: { $in: customerIds } } },
        { $group: { _id: '$customerId', total: { $sum: { $ifNull: ['$totalAmount', '$total'] } } } },
      ]);
      purchaseMap = Object.fromEntries(
        purchaseData.filter(p => p._id != null).map(p => [p._id.toString(), p.total])
      );
    } catch {
      // non-critical if Sale model has different field names
    }
  }
  ...
  const data = customers.map(c => ({
    ...c,
    id: c._id.toString(),
    totalPurchases: purchaseMap[c._id.toString()] || 0,
    balanceDue: c.balance || 0,
  }));
...
const getById = async (id, businessId) => {
  const customer = await Customer.findOne({ _id: id, businessId }).lean();
  if (!customer) throw new CustomerError('Customer not found', 404);
  return { ...customer, id: customer._id.toString(), balanceDue: customer.balance || 0 };
};
