...
const create = async (businessId, data, userId) => {
  ...
  // Normalize unit
  if (data.unit) {
    const lowered = data.unit.toLowerCase();
    data.unit = VALID_UNITS.includes(lowered) ? lowered : 'pcs';
  }

  if (data.sku) {
    const existingSku = await Product.findOne({ sku: data.sku.toUpperCase(), businessId });
    if (existingSku) throw new ProductError('SKU already exists', 409);
  }
...
const update = async (id, businessId, data) => {
  ...
  if (data.sku) {
    const existingSku = await Product.findOne({ sku: data.sku.toUpperCase(), businessId, _id: { $ne: id } });
    if (existingSku) throw new ProductError('SKU already exists', 409);
  }
  const product = await Product.findOneAndUpdate({ _id: id, businessId }, data, { new: true, runValidators: true });
  if (!product) throw new ProductError('Product not found', 404);
...
const bulkImport = async (businessId, products, userId) => {
  const created = [];
  for (const item of products) {
    try {
      const product = await create(businessId, item, userId);
      created.push(product);
    } catch (e) {
      created.push({ error: e.message, ...item });
    }
  }
  return { imported: created.filter(c => !c.error).length, failed: created.filter(c => c.error).length, results: created };
};

const bulkExport = async (businessId) => {
  const products = await Product.find({ businessId }).populate('categoryId', 'name').lean();
  return products;
};
