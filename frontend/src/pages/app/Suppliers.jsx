...
const canDelete = hasPermission('suppliers.delete');
  ...
  const fetchSuppliers = useCallback(async () => {
    setLoading(true);
    try {
      const res = await supplierApi.getAll({ page, limit, search });
      const raw = res.data;
      ...
      if (Array.isArray(raw?.data)) {
        ...
        items = raw;
      } else if (raw?.data && !Array.isArray(raw.data) && Array.isArray(raw.data.suppliers)) {
        items = raw.data.suppliers;
      }
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      const raw = res.data;
      // Stats endpoint returns { success, data: { totalSuppliers, totalPayable, totalPurchases } }
      const statsData = raw?.data || raw;
      setStats({
        totalSuppliers: statsData?.totalSuppliers ?? statsData?.total ?? 0,
        totalPayable: statsData?.totalPayable ?? statsData?.payable ?? 0,
        totalPurchases: statsData?.totalPurchases ?? statsData?.purchases ?? 0,
      });
    } catch {
      // non-critical
    }
  ...
  const openDetailModal = async (supplier) => {
    ...
    try {
      ...
      let ledgerItems = [];
      if (Array.isArray(ledgerRaw?.data)) {
        ledgerItems = ledgerRaw.data;
      } else if (Array.isArray(ledgerRaw)) {
        ledgerItems = ledgerRaw;
      } else if (ledgerRaw?.data && !Array.isArray(ledgerRaw.data) && Array.isArray(ledgerRaw.data.ledger)) {
        ledgerItems = ledgerRaw.data.ledger;
      }
      setLedgerData(ledgerItems);

      const detailRaw = detailRes.data;
      const detail = detailRaw?.data?.supplier || detailRaw?.supplier || detailRaw?.data || detailRaw;
      setSelectedSupplier({ ...supplier, ...detail });
      setPurchaseHistory(detail.purchases || []);
      setPaymentHistory(detail.payments || []);
    } catch {
  ...
  const handleTabChange = async (tab) => {
    ...
    try {
      if (tab === 'ledger') {
        const res = await supplierApi.getLedger(selectedSupplier.id);
        const raw = res.data;
        let items = [];
        if (Array.isArray(raw?.data)) {
          items = raw.data;
          ...
          items = raw;
        } else if (raw?.data && !Array.isArray(raw.data) && Array.isArray(raw.data.ledger)) {
          items = raw.data.ledger;
        }
        setLedgerData(items);
      }
    } catch {
