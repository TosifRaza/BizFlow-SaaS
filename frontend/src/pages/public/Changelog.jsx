...
const ENTRIES = [
  {
    ...
    desc: 'Added role-based access control with Owner, Manager, and Staff roles. Fine-grained permissions across 14 modules.',
  },
  {
    version: 'v1.1.0',
    date: 'July 2026',
    title: 'Expense Tracking & Reports',
    desc: 'New expense management module with category-wise breakdowns, payment tracking, and monthly trend reports.',
  },
  {
    version: 'v1.0.0',
    date: 'June 2026',
    title: 'Initial Launch',
    desc: 'Core platform launch with inventory management, POS billing, customer & supplier management, sales tracking, and GST-compliant invoicing.',
  },
];
            ...
            <div className="space-y-12">
              {ENTRIES.map((entry) => (
                <div key={entry.version} className="relative pl-12">
                  <div className="absolute left-0 top-1.5 w-[31px] h-[31px] rounded-full bg-blue-600 text-white flex items-center justify-center text-xs font-bold">{entry.version.replace('v', '').split('.')[0]}</div>
                  <div className="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md transition-shadow">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-2">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-100">{entry.version}</span>
                      <span className="text-xs text-gray-400">{entry.date}</span>
                    </div>
                    <h3 className="text-lg font-semibold text-gray-900">{entry.title}</h3>
                    <p className="mt-2 text-sm text-gray-500 leading-relaxed">{entry.desc}</p>
                  </div>
                </div>
              ))}
            </div>
