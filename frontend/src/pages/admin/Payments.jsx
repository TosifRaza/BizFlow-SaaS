  ...
  const fetchPayments = useCallback(async () => {
    try {
      ...
      if (Array.isArray(raw?.data)) {
        ...
        items = raw;
      } else if (raw?.data && !Array.isArray(raw.data) && Array.isArray(raw.data.payments)) {
        items = raw.data.payments;
      }
  ...
  const columns = [
    {
      key: 'businessId',
      label: 'Business Name',
      render: (val) => <span className="font-semibold text-gray-900">{val?.name || 'N/A'}</span>,
    },
    {
      key: 'planId',
      label: 'Plan',
      render: (val) => <span className="text-gray-600">{val?.name || 'N/A'}</span>,
    },
    {
      key: 'amount',
      label: 'Amount',
